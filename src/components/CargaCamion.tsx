  ...
  >
    ...
    <div className="flex-1 min-w-0">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300">
          {palet.estado}
        </span>
      </div>
    ...
    >
        ...
        {previewPct !== null && previewPct > pct && (
          <div
            className={`absolute top-0 h-full ${
              exceeds ? "bg-red-400/70" : "bg-brand-400/60"
            } transition-all`}
            style={{
              left: `${pct}%`,
      ...
      <div className="flex items-center justify-between mt-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400">
        ...
        {previewPct !== null && (
          <span
            className={
              exceeds ? "text-red-500" : "text-brand-500 dark:text-brand-400"
            }
          >
  ...
  return (
    <div className="min-h-full text-slate-900 dark:text-white font-sans">
      <div className="flex flex-col gap-6">
        {/* Header */}
        <div className="flex flex-wrap justify-between items-center gap-4 bg-white dark:bg-slate-900/50 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-semibold uppercase italic tracking-tight">
                Carga de camión
              </h1>
          ...
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              className="px-4 py-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm font-semibold tracking-wider outline-none focus:ring-2 focus:ring-brand-500/40 disabled:opacity-50"
            >
              {camiones.map((c) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(320px,420px)_1fr] gap-5">
          {/* PALETS PENDIENTES */}
          <section className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm flex flex-col overflow-hidden">
            <div className="p-5 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold uppercase tracking-wide">
                  Palets pendientes
                </h2>
                <span className="px-2.5 py-1 rounded-full bg-amber-100 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 text-[10px] font-semibold tracking-wider">
                  {pendientes.length}
                </span>
              </div>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-3 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-sm outline-none focus:ring-2 focus:ring-brand-500/40"
                />
              </div>
            </div>
          ...
          <section className="flex flex-col gap-4">
                      ...
                      cargaPalets.length > 0 && (
                        <div className="flex flex-col gap-3 w-full sm:w-auto sm:min-w-70">
                          ...
                          <input
                            ...
                            className="px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-sm outline-none focus:ring-2 focus:ring-brand-500/40"
                          />
                          ...
                          <ul className="flex flex-col gap-1.5 max-h-44 overflow-y-auto pr-1">
                            {paradas.map((p, idx) => (
                              ...
                              >
                                <span className="w-5 h-5 flex items-center justify-center rounded-full bg-brand-100 dark:bg-brand-950/40 text-brand-600 dark:text-brand-300 text-[10px] font-semibold">
                                  {idx + 1}
                                </span>
                          ...
                          <button
                            ...
                            className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-2xl text-xs font-semibold uppercase tracking-wide shadow-md active:scale-95 transition-all disabled:opacity-60 ${
                              confirmIniciarRuta
            ...
            <div
              ...
              onDrop={handleDrop}
              className={`relative rounded-xl border-2 border-dashed p-5 min-h-[420px] transition-all ${
                isDropHovering
                  ? dropBlocked
                    ? "bg-red-50 dark:bg-red-950/20 border-red-400"
                    : "bg-brand-50 dark:bg-brand-950/20 border-brand-400"
                  : isCamionEnRuta
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {selectedCamion && cargaPalets.length > 0 && (
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-semibold uppercase tracking-wide border transition-all active:scale-95 disabled:opacity-60 ${
                          confirmVaciar
                ...
                {loadingCargas ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                    {cargaPalets.map((p) => (
                      <div
                        key={p.docId}
                        className="group relative bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-2.5 shadow-sm hover:border-brand-400 transition-all"
                      >
                        <div className="flex items-center gap-2 mb-1.5">
                          <div className="p-1 bg-brand-100 dark:bg-brand-950/40 rounded-md text-brand-600 dark:text-brand-400">
                            <Package size={12} />
                          </div>
              ...
              {isDropHovering && (
                <div
                  className={`absolute inset-0 flex items-center justify-center pointer-events-none rounded-xl ${
                    dropBlocked ? "bg-red-500/10" : "bg-brand-500/10"
                  }`}
                >
                  <div
                    className={`px-5 py-3 rounded-2xl text-sm font-semibold uppercase tracking-wide border-2 ${
                      dropBlocked
